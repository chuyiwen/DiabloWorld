...
reactor = reactor
service = services.CommandService("loginService",runstyle= services.Service.PARALLEL_STYLE)  # Parallel 并行

def serviceHandle(target):
    '''服务处理
    @param target: func Object
    '''
    service.mapTarget(target)  # 根据函数名映射
...
def serverstart():
    ...
    factory.addServiceChannel(service)  # loginService
