...
class FFServer:
    ...
    def config(self, config, servername=None, dbconfig=None,
        ...
        if netport:#net前端
            ...
            reactor.listenTCP(netport,self.netfactory)  # 监听
            
        if webport:#http服务
            self.webroot = vhost.NameVirtualHost()
            GlobalObject().webroot = self.webroot
            reactor.listenTCP(webport, DelaySite(self.webroot))  # 监听
            
        if rootport:#分布式root节点
            self.root = PBRoot()
            rootservice = services.Service("rootservice")
            self.root.addServiceChannel(rootservice)
            reactor.listenTCP(rootport, BilateralFactory(self.root))  # 监听
            
        for cnf in self.remoteportlist:#remote节点配置列表
            rname = cnf.get('rootname')
            self.remote[rname] = RemoteObject(self.servername)#remote节点
            
        if hasdb and dbconfig:#数据库连接
            log.msg(str(dbconfig))
            dbpool.initPool(**dbconfig)  # 初始化
            
        if hasmem and memconfig:#memcached连接
            urls = memconfig.get('urls')
            hostname = str(memconfig.get('hostname'))
            mclient.connect(urls, hostname)  # 连接
